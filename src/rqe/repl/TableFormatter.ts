...
export function newTableFormatState(options: CustomFormatOptions): TableFormatState {
    return {
        options,
        attrs: new Map()
    }
...
export function formatHeader(state: TableFormatState) {
    ...
    if (state.options.indent) {
        titleEls.push(' '.repeat(state.options.indent));
        titleBarEls.push(' '.repeat(state.options.indent));
    }
...
export function printItems(state: TableFormatState, items: FormattedItem[], log: (s: string) => void) {
    for (let item of items) {
        ...
        for (let lineIndex=0; lineIndex < item.lineCount; lineIndex++) {
            ...
            if (state.options.indent)
                outputEls.push(' '.repeat(state.options.indent));
...
export function formatTable(table: Table, options: CustomFormatOptions = {}): string[] {
    let out = [];

    const formatState = newTableFormatState(options);
